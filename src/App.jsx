...
const S = {
  ...
  card: { background: "#FFFFFF", borderRadius: 16, padding: "2.5rem 2.5rem", width: "100%", maxWidth: 640, boxShadow: "0 1px 3px rgba(0,0,0,0.06), 0 4px 20px rgba(0,0,0,0.04)" },
  ...
  return (
    <div style={S.header}>
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        <div style={{ width: 36, height: 36, borderRadius: "50%", background: S.green, display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <div>
          <div style={{ fontSize: 13, fontWeight: 600, color: S.ink, letterSpacing: "-0.01em" }}>Dr. JJ Mayo</div>
          <div style={{ fontSize: 11, color: S.muted }}>GLP-1 Metabolic Rebuild</div>
        </div>
      </div>
      {step > 0 && step <= total && <div style={{ fontSize: 13, color: S.muted }}>{step} / {total}</div>}
    </div>
  );
}
...
function ProgressBar({ pct }) {
  return (
    <div style={{ height: 3, background: "#E8E6E1", borderRadius: 2, marginBottom: "2rem", overflow: "hidden" }}>
      <div style={{ height: "100%", width: `${pct}%`, background: S.green, borderRadius: 2, transition: "width 0.4s ease" }} />
    </div>
  );
}
...
function IntroScreen({ onStart }) {
  return (
    <div style={S.card}>
      <div style={{ display: "inline-block", background: S.greenLight, color: S.greenDark, fontSize: 11, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", padding: "4px 10px", borderRadius: 6, marginBottom: "1.5rem" }}>
        Free Assessment · 90 seconds
      </div>
      <h1 style={{ ...S.serif, fontSize: "clamp(26px, 5vw, 34px)", lineHeight: 1.25, color: S.ink, marginBottom: "1rem" }}>
        Why Has Your Weight Loss Stalled on GLP-1?
      </h1>
      <p style={{ fontSize: 16, color: S.muted, lineHeight: 1.7, marginBottom: "2rem" }}>
        You're paying hundreds of dollars a month for medication that worked — and now it doesn't. Answer 10 questions and find out exactly which of the 4 plateau patterns is keeping you stuck, plus a personalized 3-step protocol to break through it.
      </p>
      <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: "2rem" }}>
        {["Discover your specific plateau type", "Get a 3-step personalized protocol", "Understand why eating less made it worse"].map((item) => (
          <div key={item} style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 20, height: 20, borderRadius: "50%", background: S.greenLight, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              <svg width="10" height="8" viewBox="0 0 10 8" fill="none"><path d="M1 4L3.5 6.5L9 1" stroke={S.green} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" /></svg>
            </div>
            <span style={{ fontSize: 15, color: S.ink }}>{item}</span>
          </div>
        ))}
      </div>
      <button onClick={onStart} style={{ width: "100%", padding: "0.9rem", background: S.green, color: "white", border: "none", borderRadius: 10, fontSize: 16, fontWeight: 600, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>
        Find Out Why I'm Stuck →
      </button>
      <p style={{ fontSize: 12, color: S.faint, textAlign: "center", marginTop: "1rem" }}>No spam. No sales pitch. Just your results.</p>
    </div>
  );
}
...
function QuestionScreen({ question, stepNum, total, selected, onSelect, onNext, onBack }) {
  ...
  return (
    <div style={S.card}>
      <ProgressBar pct={pct} />
      <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", color: S.green, marginBottom: "0.5rem" }}>Question {stepNum} of {total}</div>
      <h2 style={{ ...S.serif, fontSize: "clamp(18px, 4vw, 22px)", lineHeight: 1.35, color: S.ink, marginBottom: "1.75rem", fontWeight: 400 }}>{question.text}</h2>
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {question.options.map((opt, i) => {
          const isSelected = selected === opt;
          return (
            <button key={i} onClick={() => onSelect(opt)} style={{ textAlign: "left", padding: "0.875rem 1.125rem", background: isSelected ? S.greenLight : "#FAFAF8", border: `1.5px solid ${isSelected ? S.green : "#E8E6E1"}`, borderRadius: 10, fontSize: 15, color: isSelected ? S.greenDark : S.ink, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", fontWeight: isSelected ? 500 : 400, lineHeight: 1.5 }}>
              {opt.label}
            </button>
          );
        })}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "1.75rem" }}>
        <button onClick={onBack} style={{ background: "none", border: "none", color: S.muted, fontSize: 14, cursor: "pointer", padding: 0, fontFamily: "'DM Sans', sans-serif", visibility: stepNum === 1 ? "hidden" : "visible" }}>← Back</button>
        <button onClick={onNext} disabled={!selected} style={{ padding: "0.6rem 1.5rem", background: selected ? S.green : "#E8E6E1", color: selected ? "white" : S.faint, border: "none", borderRadius: 8, fontSize: 15, fontWeight: 500, cursor: selected ? "pointer" : "default", fontFamily: "'DM Sans', sans-serif" }}>
...
function ScoreBar({ label, pct, isWinner, color }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
      <div style={{ fontSize: 12, color: isWinner ? S.ink : S.muted, fontWeight: isWinner ? 600 : 400, minWidth: 140 }}>{label}</div>
      <div style={{ flex: 1, height: 6, background: "#E8E6E1", borderRadius: 3, overflow: "hidden" }}>
        <div style={{ height: "100%", width: `${pct}%`, background: isWinner ? color : "#C8C5BF", borderRadius: 3, transition: "width 0.8s ease" }} />
      </div>
      <div style={{ fontSize: 12, color: isWinner ? S.ink : S.faint, minWidth: 32, textAlign: "right", fontWeight: isWinner ? 600 : 400 }}>{pct}%</div>
    </div>
  );
}
  ...
  return (
    <div style={{ width: "100%", maxWidth: 640 }}>
      <div style={{ ...S.card, borderTop: `4px solid ${t.borderColor}`, marginBottom: "1.25rem" }}>
        ...
        <div style={{ background: "#FAFAF8", borderRadius: 10, padding: "1rem 1.25rem" }}>
          <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: "0.07em", textTransform: "uppercase", color: S.muted, marginBottom: "0.75rem" }}>How your answers scored</div>
          {Object.entries(typeLabels).map(([k, label]) => {
            const pct = maxScore > 0 ? Math.round((totals[k] / maxScore) * 100) : 0;
            return <ScoreBar key={k} label={label} pct={pct} isWinner={k === result} color={t.color} />;
          })}
        </div>
      </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {t.steps.map((step, i) => (
            <div key={i} style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
              <div style={{ minWidth: 28, height: 28, borderRadius: "50%", background: t.bgLight, color: t.color, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, flexShrink: 0 }}>{i + 1}</div>
              <div>
                <div style={{ fontSize: 15, fontWeight: 600, color: S.ink, marginBottom: "0.3rem" }}>{step.title}</div>
                <div style={{ fontSize: 14, color: S.muted, lineHeight: 1.65 }}>{step.body}</div>
              </div>
            </div>
          ))}
        </div>
        <div style={{ background: t.bgLight, borderLeft: `3px solid ${t.borderColor}`, borderRadius: "0 8px 8px 0", padding: "0.875rem 1rem", marginTop: "1.5rem", fontSize: 14, color: t.color, lineHeight: 1.6 }}>
      ...
      <div style={{ ...S.card, background: S.greenDark, marginBottom: "1rem" }}>
        ...
        <p style={{ fontSize: 14, color: "rgba(255,255,255,0.7)", lineHeight: 1.65, marginBottom: "1.5rem" }}>Dr. JJ Mayo helps women over 40 break through their GLP-1 plateau — losing fat, protecting muscle, and getting confident about how to eat and move on medication. Real results, not generic advice.</p>
